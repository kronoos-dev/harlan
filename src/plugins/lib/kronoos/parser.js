...
export class KronoosParse {
    ...
    cbuscaTelefone() {
        ...
        let ktable = this.firstElement().captionTable("Telefones");
        //_.each(_.uniq(telefones.map((i, e) => VMasker.toPattern($("ddd", e).text() + $("numero", e).text(), "(99) 9999-99999")).toArray()), (e) => klist(e));
        for(let i = 0; i <= telefones.length;i=i+3){

          telefones = _.uniq(telefones.map((i, e) => VMasker.toPattern($("ddd", e).text() + $("numero", e).text(), "(99) 9999-99999")).toArray());

          ktable(telefones[i], telefones[i+1], telefones[i+2]);

          if(telefones.length > 3){
            ktable(telefones[i+3]);
          }
        }
